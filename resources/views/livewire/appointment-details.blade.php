                    ...
                    <div class="bg-gray-100 rounded-lg p-4 mb-4 border-l-4 {{ $index === 0 ? 'border-blue-500' : 'border-gray-300' }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6">
                            <div class="space-y-4">
                                ...
                                <div class="bg-white rounded-lg p-4 border border-gray-300">{{ $guest->name }}</div>

                                <span class="text-gray-600 font-medium">Email</span>
                                <div class="bg-white rounded-lg p-4 border border-gray-300">{{ $guest->email }}</div>
                            </div>
                            <div class="space-y-4">
                                ...
                                <div class="bg-white rounded-lg p-4 border border-gray-300">
                                    @php
                                        ...
                                        $phoneFormatted = strlen($phone) > 6
                                            ...
                                            : $phone; // Jika panjang kurang dari atau sama dengan 6, tampilkan apa adanya
                                    @endphp
                                    {{ $phoneFormatted }}
                                </div>
                                ...
                                <div class="bg-white rounded-lg p-4 border border-gray-300">
                                    @php
                                        ...
                                        $identityFormatted = strlen($identity) > 6
                                            ...
                                            : $identity; // Jika panjang kurang dari atau sama dengan 6, tampilkan apa adanya
                                    @endphp
                                    {{ $identityFormatted }}
                                </div>
                            </div>
                ...
                <div class="bg-gray-100 rounded-lg p-6 border-l-4 border-blue-500">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                        <div class="space-y-2">
                            ...
                            <div class="bg-white rounded-lg p-3 border border-gray-300 text-gray-800">{{ $guest->organization }}</div>
                        </div>
                        <div class="space-y-2">
                            <span class="text-gray-600 font-medium block">Tanggal Check-In</span>
                            <div class="bg-white rounded-lg p-3 border border-gray-300 text-gray-800">{{ \Carbon\Carbon::parse($appointment->check_in)->format('d M Y H:i') }}</div>
                        </div>
                        <div class="space-y-2">
                            <span class="text-gray-600 font-medium block">Bertemu Dengan</span>
                            <div class="bg-white rounded-lg p-3 border border-gray-300 text-gray-800">{{ $appointment->host->name ?? 'Tidak Diketahui' }}</div>
                        </div>
                        <div class="space-y-2">
                            <span class="text-gray-600 font-medium block">Tanggal Check-Out</span>
                            <div class="bg-white rounded-lg p-3 border border-gray-300 text-gray-800">{{ \Carbon\Carbon::parse($appointment->check_out)->format('d M Y H:i') }}</div>
                        </div>
                        <div class="space-y-2 sm:col-span-2">
                            <span class="text-gray-600 font-medium block">Keperluan</span>
                            <div class="bg-white rounded-lg p-3 border border-gray-300 text-gray-800 break-words whitespace-normal">
                                {{ $appointment->needs }}
                            </div>
                        </div>
                    </div>
